...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //  something more generic
    ...
    size_t pos = sql.find(" or ");
    while (pos != std::string::npos) {
        size_t start = pos + 4;
        size_t equals_pos = sql.find("=", start);

        if (equals_pos != std::string::npos) {
            std::string left = sql.substr(start, equals_pos - start);
            std::string right = sql.substr(equals_pos + 1);

            // it wasn't finding the injection without removing the space and slash
            // im sure there is a better way to do this
            left.erase(left.find_last_not_of(" '\";") + 1);
            right.erase(right.find_last_not_of(" '\";") + 1);

            if (left == right) {
                std::cout << "WARNING: SQL injection attack detected!\n";
                return false;
            }
        }
    }
